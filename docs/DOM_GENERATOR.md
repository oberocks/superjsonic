...
4   | `subType`  | No       | String        | This argument is REQUIRED when the 2nd argument is `'insert'` or `'replace'`!  <br><br> If you are using `'insert'` or `'replace'`, then this argument should be either `'before'` or `'after'` (for `.insertBefore()` or `.insertAfter()` functionality). If the 2nd argument is `'replace'`, then the 4th argument can be any string (even an empty string) and it will work. That said, it's recommended to use the string `'with'` as a 4th argument for `'replace'`, because it's super easy to read afterwards. <br><br> Don't forget, all passed strings are **Case Insensitive**!
